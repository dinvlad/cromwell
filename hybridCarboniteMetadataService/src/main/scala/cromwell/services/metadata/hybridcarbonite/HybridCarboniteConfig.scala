...
final case class HybridCarboniteConfig(enabled: Boolean, pathBuilders: PathBuilders, bucket: String, freezeScanConfig: HybridCarboniteFreezeScanConfig) {
...
object HybridCarboniteConfig {
  ...
  def parseConfig(carboniterConfig: Config)(implicit system: ActorSystem): Checked[HybridCarboniteConfig] = {
    ...
    } yield HybridCarboniteConfig(enable, pathBuilders, bucket, freezeScan)
  }
}
